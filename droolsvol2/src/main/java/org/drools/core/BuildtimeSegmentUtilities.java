...
import org.drools.core.SegmentMemory.EvalMemoryPrototype;
import org.drools.core.SegmentMemory.FromMemoryPrototype;
import org.drools.core.SegmentMemory.LiaMemoryPrototype;
import org.drools.core.SegmentMemory.MemoryPrototype;
import org.drools.core.SegmentMemory.QueryMemoryPrototype;
import org.drools.core.SegmentMemory.ReactiveFromMemoryPrototype;
import org.drools.core.SegmentMemory.RightInputAdapterPrototype;
import org.drools.core.SegmentMemory.SegmentPrototype;
import org.drools.core.SegmentMemory.TerminalPrototype;
import org.drools.core.SegmentMemory.TimerMemoryPrototype;
...
public class BuildtimeSegmentUtilities {
    ...
    public static SegmentPrototype[] createLeftTupleNodeProtoMemories(SegmentPrototypeRegistry segmentPrototypeRegistry,
                                                                      BaseNode lts,
                                                                      TerminalNode removingTn) {
        BaseNode segmentRoot = lts;
        BaseNode segmentTip = lts;
        List<SegmentPrototype> smems = new ArrayList<>();
        int start = 0;

        BaseNode firstConditional = getFirstConditionalBranchNode(lts);
    ...
    static BaseNode getFirstConditionalBranchNode(BaseNode tupleSource) {
        BaseNode conditionalBranch = null;
    ...
    private static SegmentPrototype createSegmentMemory(SegmentPrototypeRegistry segmentPrototypeRegistry,
                                                       BaseNode segmentRoot,
                                                       BaseNode segmentTip,
                                                       TerminalNode removingTn,
                                                       int recordBefore) {
        BaseNode node = segmentRoot;
        ...
        smem.setNodesInSegment(nodes.toArray(new BaseNode[nodes.size()]));
     ...
     * if the rule had already been removed from the network.
     */
    public static boolean isTipNode(BaseNode node, TerminalNode removingTN) {
    ...
    public static BaseNode findSegmentRoot(BaseNode tupleSource) {
        return findSegmentRoot(tupleSource, null);
    }

    public static BaseNode findSegmentRoot(BaseNode tupleSource, TerminalNode ignoreTn) {
